...
int main() //int argc, char *argv[]
{
	...
	serverIP = "192.168.56.101";//argv[1];
	serverPort =5555;// atoi(argv[2]);
	iterCount = 10;//atoi(argv[3]);
	iterDelay = 1;//atoi(argv[4]);
	msgSize = 9;//atoi(argv[5]);    ----> the default message size with only headers and no DATA is '9'! If you want to add data by changing the Null status assigned to DATA while calling 
                                         //    createMessage() ,  don't forget to change the size over here. 
	burstSize = 2;//atoi(argv[6]);
	...
	for (seq = 0; seq < iterCount && !quit; ++seq) {
		...
		for (i = 0; i < burstSize && !quit; ++i) {
			...
			createMessage(msg, cmd, sessionID, seq+1, NULL, msgSize);  // If you want to add data , change the NULL value to some string and update the size of msgsize variable above
                        //printf("%s\n",msg);
			sendto(sock, msg, strlen(msg), 0, (struct sockaddr *)&serverAddr, sizeof(serverAddr));
                       // printf("%s\n",msg);
		}
...
void displayResults(char *res)
{
	...
	char *str[] = { "Available Bandwidth (Mbps)", 
			...
			"Total Bursts" };
        FILE *fp;
        fp = fopen("fbitrate.txt" , "r+");
	res = strtok(res, ":");				/* retrieve tokens from results */
	while (res) {
	
		printf("%s = %s\n", str[i++], res);
                // put estimated bandwidth into a file 
